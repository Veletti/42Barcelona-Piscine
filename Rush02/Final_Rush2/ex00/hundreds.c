...
/*   Updated: 2022/07/24 16:53:14 by rvela-fe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_write_mils(char *dic, int div, char *num)
{
	int	i;
	int	c;
	int	done;
	...
	numzeros = div * 3;
	{
		while (dic[i] != '\0')
		{
			...
			{
				i++;
				go_write_mils(int c, int numzeros, char *dic);
			}
			i++;
			if (done == 1)
				break ;
...
void	go_write_mils(int c, int numzeros, char *dic)
{
	c = 0;
	while (c < numzeros)
	{
